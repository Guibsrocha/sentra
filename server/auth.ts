...
/**
 * Middleware de autenticação para Express.js
 * Verifica token JWT no cookie e adiciona usuário ao req.user
 */
export async function authenticateUser(
  ...
  try {
    ...
    if (!user) {
      console.log('[Auth] Usuário não encontrado:', payload.userId);
      res.status(401).json({
        success: false,
